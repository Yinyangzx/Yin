...
local ThemeBackgroundImages = {
    J1 = "",
    Dark = "rbxassetid://76341519079058",
...
--// Forzar imagen de fondo del tema Dark después de LoadThemes,
--// para que el repo externo no la sobreescriba.
ThemeBackgroundImages["Dark"] = "rbxassetid://76341519079058"
...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    --// ════════════════════════════════════════════════════════════════════════
    --// BANNER SLIDESHOW — Imagen pequeña junto al nombre de la librería
    --// ════════════════════════════════════════════════════════════════════════
    --// Muestra imágenes que ciclan instantáneamente en el TopBar, justo a la
    --// derecha del nombre de la librería ("Yin Yang"). Sin tweens ni fade.
    --//
    --// ── CÓMO AGREGAR / QUITAR BANNERS ────────────────────────────────────
    --// Editá la tabla BannerSlideshow_Images. Cada entrada tiene:
    --//
    --//   { name = "NombreUnico.png", url = "URL_directa_de_la_imagen" }
    --//
    --//   • name  → nombre del archivo local que se guarda en disco (caché).
    --//             Debe ser único y sin espacios. Ejemplo: "YYBannerCat1.png"
    --//   • url   → URL directa (raw) de la imagen.
    --//             En GitHub usá raw.githubusercontent.com, NO github.com.
    --//             Ejemplo correcto:
    --//             "https://raw.githubusercontent.com/usuario/repo/main/img.png"
    --//             Ejemplo incorrecto (no usar):
    --//             "https://github.com/usuario/repo/blob/main/img.png"
    --//
    --// ── PARÁMETROS AJUSTABLES ─────────────────────────────────────────────
    --//   BannerSlideshow_Interval  → segundos que cada imagen permanece visible
    --//                               (default: 2)
    --//
    --// ── REQUISITOS DEL EXECUTOR ───────────────────────────────────────────
    --// Requiere que el executor soporte estas funciones globales:
    --//   game:HttpGet, writefile, isfile, getcustomasset
    --// Si alguna falta, el slideshow se omite sin romper el resto de la UI.
    --// ════════════════════════════════════════════════════════════════════════

    local BannerSlideshow_Images = {
        { name = "YYBannerCat1.png", url = "https://raw.githubusercontent.com/Yinyangzx/Yin-Imag/main/BackgroundEraser_20260924_191300955.png" },
        { name = "YYBannerCat2.png", url = "https://raw.githubusercontent.com/Yinyangzx/Yin-Imag/main/BackgroundEraser_20260924_190941541.png" },
    }
    local BannerSlideshow_Interval = 1

    local BannerFrame = mk("Frame", {
        Name            = "BannerSlideshow",
        AnchorPoint     = Vector2.new(0, 0.5),
        Position        = UDim2.new(0, 98, 0.5, 0),
        Size            = UDim2.fromOffset(30, 30),
        BackgroundTransparency = 1,
        ZIndex          = 8,
    }, TopBar)
    corner(BannerFrame, 6)

    --// Double buffer: dos labels superpuestos. Mientras uno se muestra,
    --// el otro carga la siguiente imagen en silencio → cambio sin parpadeo.
    local BannerImageA = mk("ImageLabel", {
    ...
    corner(BannerImageA, 6)

    local BannerImageB = mk("ImageLabel", {
        Size                   = UDim2.fromScale(1, 1),
        BackgroundTransparency = 1,
        Image                  = "",
        ScaleType              = Enum.ScaleType.Fit,
    ...
    corner(BannerImageB, 6)

    task.spawn(function()
        --// Verificar soporte del executor antes de intentar cualquier carga
        if type(game.HttpGet) ~= "function"
        or type(writefile) ~= "function"
        or type(isfile) ~= "function"
        or type(getcustomasset) ~= "function" then return end

        --// Pre-cargar todas las imágenes en paralelo (descarga + caché en disco)
        local assetCache = {}
        for _, banner in ipairs(BannerSlideshow_Images) do
            task.spawn(function()
                pcall(function()
                    --// Si el archivo ya existe en disco, saltar la descarga
                    local cachedOk, cachedExists = pcall(isfile, banner.name)
                    if not (cachedOk and cachedExists) then
                        local dlOk, data = pcall(game.HttpGet, game, banner.url, true)
                        if not dlOk or type(data) ~= "string" or #data < 10 then return end
                        pcall(writefile, banner.name, data)
                    end
                    --// Convertir el archivo local en un asset Roblox-compatible
                    local assetOk, assetId = pcall(getcustomasset, banner.name)
                    if assetOk and type(assetId) == "string" then
                        assetCache[banner.name] = assetId
                    end
                end)
            end)
        end

        --// Esperar hasta que el primer banner esté listo (máximo 10 segundos)
        local waited = 0
        while waited < 10 and not assetCache[BannerSlideshow_Images[1].name] do
            task.wait(0.25)
            waited = waited + 0.25
        end
        if not assetCache[BannerSlideshow_Images[1].name] then return end

        --// Double buffer: pre-cargar las dos primeras imágenes antes de mostrar
        local function getAsset(i)
            local b = BannerSlideshow_Images[((i - 1) % #BannerSlideshow_Images) + 1]
            return assetCache[b.name] or ""
        end

        BannerImageA.Image = getAsset(1)
        BannerImageB.Image = getAsset(2)
        task.wait(0.05)  --// pausa mínima para que Roblox registre ambas texturas

        local front = BannerImageA   --// el label que se ve ahora
        local back  = BannerImageB   --// el label que carga la siguiente en silencio
        front.Visible = true
        back.Visible  = false

        --// Ciclo: el back ya tiene la siguiente imagen lista → swap instantáneo
        local idx = 1
        while BannerFrame and BannerFrame.Parent do
            task.wait(BannerSlideshow_Interval)

            --// Mostrar el back (ya cargado) y ocultar el front
            back.Visible  = true
            front.Visible = false
            front, back = back, front  --// intercambiar roles

            --// Avanzar índice y pre-cargar la próxima imagen en el back
            idx = (idx % #BannerSlideshow_Images) + 1
            local nextIdx = (idx % #BannerSlideshow_Images) + 1
            back.Image = getAsset(nextIdx)
        end
    end)
    ...
    local BoomboxPill = mk("Frame", {
        ...
        Position = UDim2.new(0.5, -77, 0.5, -18),
    ...
    function Window:CreateTab(nameSpanish, nameEnglishOrIcon, iconAsset)
        ...
        local function clampTabScroll()
            task.defer(function()
                if not TabPage.Parent then return end
                local maxY = math.max(0, TabPage.AbsoluteCanvasSize.Y - TabPage.AbsoluteWindowSize.Y)
                if TabPage.CanvasPosition.Y > maxY then
                    TabPage.CanvasPosition = Vector2.new(TabPage.CanvasPosition.X, maxY)
                end
            end)
        end
        TabPage:GetPropertyChangedSignal("AbsoluteCanvasSize"):Connect(clampTabScroll)
        TabPage:GetPropertyChangedSignal("AbsoluteWindowSize"):Connect(clampTabScroll)

        local Tab = {Button = TabButton, Page = TabPage, NameSpanish = nameSpanish, NameEnglish = nameEnglish, IconAsset = iconAsset}

        --// Contenedor de contenido opcional. Por defecto todo sigue usando TabPage.
        Tab._ContentParent = TabPage

        function Tab:CreateContentContainer(name)
            local container = mk("Frame", {
                Name = name or "ContentContainer",
                Size = UDim2.new(1, 0, 0, 0),
        ...
        function Tab:SetContentContainer(container)
            self._ContentParent = container or TabPage
            return self._ContentParent
        end

        function Tab:GetContentContainer()
            return self._ContentParent or TabPage
        end

        function Tab:ResetContentContainer()
            self._ContentParent = TabPage
            return TabPage
        end
        ...
        function Tab:CreateButton(textSpanish, textEnglishOrCallback, callbackOrIcon, iconAsset)
            local contentParent = self._ContentParent or TabPage
            --// COMPATIBILIDAD: si el 2do argumento es string, es modo bilingüe.
            ...
            if contentParent == TabPage then resetScrollTop(TabPage) end
...
--// ══════════════════════════════════════════════════════════════════════════════
    ...
    function Window:InitializeShaderSystem()
    ...
    --// Colgado en Window.* (no como 'local'/'local function') porque este scope ya está
    --// al límite de 200 locales de Lua — ver nota de línea 14368. Agregar 2 locals nuevos
    --// acá era lo que rompía la carga del script sin error visible en consola.
    Window.RenderedChatFrames = {}

    Window.TrackChatFrame = function(frame)
    	while #Window.RenderedChatFrames >= MAX_MESSAGES do
    		local oldest = table.remove(Window.RenderedChatFrames, 1)
    		if oldest and oldest.Parent then
    			oldest:Destroy()
    		end
    	end
    	table.insert(Window.RenderedChatFrames, frame)
    end
    ...
    Window.RenderInviteCard = function(playerName, userId, timeStamp, isSelf, inviteJobId)
    	...
    	Window.TrackChatFrame(MessageFrame)
	...
	-- DETECCIÓN DE STICKER
    	...
    	if stickerAsset then
    		...
    		Window.TrackChatFrame(StickerFrame)
    ...
    --// ════════════════════════════════════════════════════════════════
    --// CHAT: SISTEMA DE STICKERS (aislado para no consumir locales de CreateWindow)
    --// ════════════════════════════════════════════════════════════════
    function Window:InitializeChatStickers()
    -- PANEL DE STICKERS
    ...
    --// ════════════════════════════════════════════════════════════════
    --// ════════════════════════════════════════════════════════════════
    --// TAB: CRÉDITOS (aislada para no consumir locales de CreateWindow)
    --// ════════════════════════════════════════════════════════════════
    ...
    local function normalizeTrack(track, index)
        ...
        -- Método 2 de portada: URL real (ej. GitHub) que se descarga y cachea como archivo propio,
        -- igual que el audio. Si CoverURL está vacío, se sigue usando "cover" (rbxassetid) de arriba.
        local coverDownloadUrl = track.CoverURL or track.coverURL or track.CoverUrl or ""
        ...
        local coverCacheName = track.CoverCacheName or track.coverCacheName or ""
        if trim(coverDownloadUrl) ~= "" and trim(coverCacheName) == "" then
            -- BUGFIX: antes se guardaba siempre como "_cover.png" sin importar el archivo real,
            -- lo que rompía la decodificación cuando la URL era .jpg/.jpeg/.webp (el executor
            -- decide el formato de imagen por la extensión del nombre de archivo cacheado).
            -- Ahora se toma la extensión real de la URL (ignorando ?query o #fragment);
            -- si no se puede determinar una extensión razonable, se cae a .png.
            local urlWithoutQuery = tostring(coverDownloadUrl):match("^[^%?#]+") or tostring(coverDownloadUrl)
            local urlExt = urlWithoutQuery:match("%.([%w]+)$")
            urlExt = (urlExt and urlExt ~= "" and #urlExt <= 5) and urlExt:lower() or "png"
            coverCacheName = safeName .. "_cover." .. urlExt
        end
        ...
        return {
            ...
            CoverURL = tostring(coverDownloadUrl),
            CoverCacheName = tostring(coverCacheName),
    ...
    local function updateTrackRow(rowData, index)
        ...
        if rowData.Cover then
            rowData.Cover.Image = resolveTrackCoverImage(track)
        end
    ...
    local function updatePlayerFromTrack(track, index, statusText)
        ...
        local resolvedCover = resolveTrackCoverImage(track)
        local hasCover = trim(resolvedCover or "") ~= ""

        NowPlayingBackdrop.Image = hasCover and resolvedCover or ""
        ...
        AlbumArt.Image = hasCover and resolvedCover or ""
    ...
    -- Método 2 de portada: descarga y cachea la imagen real (ej. GitHub) como archivo propio,
    -- con el mismo patrón que ensureTrackCached usa para el audio (isfile / writefile / getcustomasset).
    -- Se guarda como archivo suelto (sin carpeta), con prefijo "YinYang_Cover_" para no chocar
    -- nunca con los nombres de caché de audio (CacheName).
    local function ensureCoverCached(track)
        if not track.CoverURL or track.CoverURL == "" then
            return false, "no_cover_url"
        end

        local cacheName = track.CoverCacheName
        if not cacheName or cacheName == "" then
            return false, "no_cover_cache_name"
        end

        local coverPath = "YinYang_Cover_" .. cacheName
        local coverExists = false

        if isfile then
            local okFile, resultFile = pcall(function()
                return isfile(coverPath)
            end)
            coverExists = okFile and resultFile == true
        end

        if not coverExists then
            if not writefile then
                return false, "writefile_unavailable"
            end

            local okDownload, data = pcall(function()
                return game:HttpGet(track.CoverURL, true)
            end)

            if not okDownload or type(data) ~= "string" or #data < 10 then
                warn("[YinYang Spotify] Falló la descarga de portada (" .. tostring(track.CoverURL) .. "): " .. tostring(data))
                return false, "download_failed"
            end

            local okWrite = pcall(function()
                writefile(coverPath, data)
            end)

            if not okWrite then
                warn("[YinYang Spotify] Falló writefile para la portada: " .. coverPath)
                return false, "cache_write_failed"
            end
        end

        if not getcustomasset then
            warn("[YinYang Spotify] getcustomasset no está disponible en este ejecutor; no se puede mostrar la portada: " .. coverPath)
            return false, "getcustomasset_unavailable"
        end

        local okAsset, resultAsset = pcall(function()
            return getcustomasset(coverPath)
        end)

        if not (okAsset and type(resultAsset) == "string") then
            warn("[YinYang Spotify] getcustomasset falló para la portada (" .. coverPath .. "): " .. tostring(resultAsset))
            return false, "getcustomasset_failed"
        end

        return true, resultAsset
    end

    -- Caché en memoria: evita repetir isfile/getcustomasset en cada refresco de fila
    -- (updateTrackRow se llama por cada fila visible en cada re-render, no solo una vez como el audio).
    local CoverAssetResolvedCache = {}

    -- Único punto de entrada para pintar la portada de un track: decide entre
    -- método 1 (rbxassetid en track.Cover, comportamiento original sin cambios)
    -- y método 2 (track.CoverURL descargada y cacheada). Si el método 2 falla,
    -- cae de vuelta a track.Cover para no romper nada.
    resolveTrackCoverImage = function(track)
        if not track then
            return ""
        end

        if track.CoverURL and track.CoverURL ~= "" then
            local cacheKey = (track.CoverCacheName and track.CoverCacheName ~= "") and track.CoverCacheName or track.CoverURL
            local cached = CoverAssetResolvedCache[cacheKey]
            if cached then
                return cached
            end

            local okCover, coverAssetOrErr = ensureCoverCached(track)
            if okCover then
                CoverAssetResolvedCache[cacheKey] = coverAssetOrErr
                return coverAssetOrErr
            end
        end

        return track.Cover or ""
    end
    ...
    refreshSpotifyFloatingPlayer = function()
        ...
        local floatResolvedCover = resolveTrackCoverImage(currentTrack)
        FloatCover.Image = floatResolvedCover or ""; FloatFallback.Visible = trim(floatResolvedCover or "") == ""
    ...
    createSongRow = function(track, index, layoutOrder)
        ...
        local Cover = mk("ImageLabel", {
            ...
            BackgroundTransparency = 0.04,
            BorderSizePixel = 0,
            Image = resolveTrackCoverImage(track),
            ScaleType = Enum.ScaleType.Crop,
            ZIndex = 12,
        })
        corner(Cover, 10)
