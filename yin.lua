...
local ThemeSlideshow    = {}  --// Temas con múltiples imágenes (slideshow)
    ...
    if rawData then
        ...
        if parseOk and type(data) == "table" and data.Themes then
            ...
            for name, theme in pairs(data.Themes) do
                ...
                --// SLIDESHOW: guardar imágenes si el tema las tiene
                if theme.Images and #theme.Images > 1 then
                    ThemeSlideshow[name] = {
                        Images   = theme.Images,
                        Interval = tonumber(theme.ImageInterval) or 5,
                    }
                end
                count = count + 1
            end
...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    function Window:SetTheme(themeName)
        ...
        if BackgroundArt then
            pcall(function()
                local slideData = ThemeSlideshow[themeName]

                if slideData then
                    --// TEMA CON SLIDESHOW
                    local token    = self._slideshowToken
                    local images   = slideData.Images
                    local interval = slideData.Interval
